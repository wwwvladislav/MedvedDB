...
mdv_hashmap * mdv_routes_find(mdv_topology *topology, mdv_uuid const *src)
{
    ...
    mdv_mstnode *mst_nodes = mdv_alloc(mdv_vector_size(toponodes) * sizeof(mdv_mstnode));
    ...
    mdv_rollbacker_push(rollbacker, mdv_free, mst_nodes);

    mdv_hashmap_foreach(unique_nodes, mdv_router_node, node)
        mst_nodes[node->idx].data = mdv_vector_at(toponodes, node->id);

    mdv_mstlink *mst_links = mdv_alloc(mdv_vector_size(topolinks) * sizeof(mdv_mstlink));
    ...
    mdv_rollbacker_push(rollbacker, mdv_free, mst_links);
